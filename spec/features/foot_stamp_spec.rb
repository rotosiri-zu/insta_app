...
RSpec.feature "FootStamps", type: :feature do
  ...
  scenario "should be created if user's config is true" do
    ...
    }.to change{FootStamp.where(to_user_id: false_user.id, from_user_id: true_user.id).to_a.size}.by(1)
  end
  ...
  scenario "should not be created if user's config is false" do
    ...
    }.to change{FootStamp.where(to_user_id: false_user.id, from_user_id: true_user.id).to_a.size}.by(0)
  end
end
